 ...
 * Date: 7/1/14.
 */
public class Picker extends ScrollView {
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            Picker.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    Picker.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            Picker.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    Picker.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    Picker.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
...
//                UIUtils.setViewSize(itemView, 0, itemHeight);
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                Picker.this.smoothScrollTo(0, p * itemHeight);
            }
        });
...
//            mDownX2 = ev.getX();
//            Log.d("aaa1","   "+Math.abs(mDownX1 - mDownX2));
//            if ( Math.abs(mDownX1 - mDownX2)>=100) {
//                mCanScroll = false;
//            }else{
//                mCanScroll = true;
//            }
...
//            startScrollerTask();
////            mDownX2 = ev.getX();
////            Log.d("aaa","   "+Math.abs(mDownX1 - mDownX2));
////            if ( Math.abs(mDownX1 - mDownX2)>=30) {
////                mCanScroll = false;
////            }else{
////                mCanScroll = true;
////            }
