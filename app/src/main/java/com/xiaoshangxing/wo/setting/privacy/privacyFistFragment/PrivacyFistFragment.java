package com.xiaoshangxing.wo.setting.privacy.privacyFistFragment;
 ...
 */
public class PrivacyFistFragment extends BaseFragment implements View.OnClickListener, IBaseView {
    @Bind(R.id.back)
    LinearLayout back;
    @Bind(R.id.title)
    TextView title;
    ...
    private void upload(final List<String> add, final List<String> remove) {
        ...
        if (!add.isEmpty() && !remove.isEmpty()) {
            OperateUtils.SchoolCirclrPermisson(add, NS.BLOCK_CODE, this, new SimpleCallBack() {
                @Override
                public void onSuccess() {
                    OperateUtils.SchoolCirclrPermisson(remove, NS.REMOVE_BLOCK_CODE, PrivacyFistFragment.this, simpleCallBack);
                }
